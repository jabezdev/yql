...
import { isAdmin, isStaffRole } from "../core/constants";
...
export async function canAccessProcess(
    ...
    // Default: staff (manager+) can access
    return isAdmin(roleSlug) || isStaffRole(roleSlug);
}
...
export async function getVisibleStageIds(
    ...
    if (isAdminUser) return null; // null means "all"
