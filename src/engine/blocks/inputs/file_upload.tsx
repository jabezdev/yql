...
export const ParticipantView: React.FC<ApplicantViewProps> = ({ block, value, onChange, readOnly }) => {
    ...
    const generateUploadUrl = useMutation(api.core.files.generateUploadUrl);
    const saveFile = useMutation(api.core.files.saveFile);
    // If value exists, fetching metadata might be nice to show filename, but storageId is opaque.
    // We can show "File Uploaded" stat.
    const fileData = useQuery(api.core.files.getFileMetadata, value ? { storageId: value } : "skip");
...
export const ReviewerView: React.FC<ReviewerViewProps> = ({ block, applicantValue }) => {
    ...
    const fileData = useQuery(api.core.files.getFileUrl, applicantValue ? { storageId: applicantValue } : "skip");
